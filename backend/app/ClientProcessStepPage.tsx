...
const ClientProcessStepPage: React.FC = () => {
  const { id } = useParams();
  const [data, setData] = useState<StepZoomResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const resp = await fetch(`/api/internal/process-overview/step/${id}`);
        if (!resp.ok) {
          throw new Error("Load step: " + resp.status);
        }
        const json = (await resp.json()) as StepZoomResponse;
        setData(json);
      } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">Process step details</h1>

      {id && (
        <div className="text-sm text-slate-700">
          Step id: <span className="font-mono">{id}</span>
        </div>
      )}
      ...
      {!loading && !error && !data && (
        <div className="text-sm text-slate-700">No step data.</div>
      )}
    </div>
  );
};

export default ClientProcessStepPage;
