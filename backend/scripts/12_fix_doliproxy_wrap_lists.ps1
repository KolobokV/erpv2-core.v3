...
Write-Host "=== ERPv2 backend: doliproxy wrap lists into objects ==="
...
$backupDir = Join-Path $Root ("_savepoints_backend_fix_shape_" + $timestamp)
...
Write-Host "[OK] New doliproxy.py written (SafeMode + wrapped objects)."
Write-Host ""
Write-Host "Now run backend rebuild script, for example:"
Write-Host "  cd `"$Root`""
Write-Host "  powershell -ExecutionPolicy Bypass -File .\scripts\11_backend_rebuild_after_doliproxy_fix.ps1"
Write-Host ""
Write-Host "=== Done (doliproxy wrap lists) ==="
